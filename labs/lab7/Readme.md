...
## Описание выбора сегмента ESI и MAC с LACP SYSTEM-ID:
...
## Пояснение к выбору схемы подключения клиента VPC3:

Клиент VPC3 не поддерживает два сетевых интерфейса, поэтому был подключен через коммутатор SW01. Коммутатор подключен через Port-Channel 1 к двум LEAF 1-2 и 1-3. 

На них соответсвенно настрое Port-Channel 3 с использование EVPN ESI.

## Схема сети:

![alt text](Net_Scheme_Lab7.png)
